...
async function step1MicroCeph(confObj) {
  ...
  const cmds_inst = [
    ...
    `apt update -y`,
    `snap refresh`,
    `snap install microceph`, // addon: rook-ceph since 1.28
    `snap refresh --hold microceph`, // prevent microceph from being updated automatically
  ];
